...
async function sharedFetchMe(force: boolean = false) {
  const now = Date.now();
  if (meInFlight) return;
  const minIntervalMs = hasLoadedOnce ? 10_000 : 3000;
  if (!force && now - lastMeAt < minIntervalMs) return;
  meInFlight = true;
  lastMeAt = now;
  try {
    if (!hasLoadedOnce) {
