    ...
    while True:
        ...
        for it in contents:
            try:
                ...
                # Идеальная логика фильтрации ZIP:
                # 1. Если задан suffix .zip, ищем файлы заканчивающиеся на .zip ИЛИ папки, содержащие .zip/
                # 2. Если файл сам является .zip, он подходит.
                if suf:
                    if zip_mode:
                        # Поддерживаем и файлы .zip, и структуру префиксов некоторых загрузчиков
                        if not (kl.endswith(".zip") or ".zip/" in kl):
                            continue
                    else:
                        if not kl.endswith(suf):
                            continue
                ...
                out.append(
                    {
                        ...
                        "etag": str((it or {}).get("ETag") or "").replace('"', ''),
                    }
                )
            except Exception:
                continue
            # Не ограничиваем сбор на этапе цикла, если нам нужна сортировка по дате позже
            # Но ограничиваем общим разумным пределом (например, 1000 объектов для админки)
            if len(out) >= 1000:
                break
